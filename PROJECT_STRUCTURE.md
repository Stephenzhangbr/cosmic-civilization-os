# Project Structure

## Overview

This repository is organized as a **layered architecture for a civilization-scale operating system**.

Each directory represents a core layer required to maintain stability, coordination, and long-term evolution in a multi-intelligence environment.

---

## System Layers

```
00_AXIOMS
01_CONSENSUS
02_OPERATING_SYSTEM
03_NCM
```

---

## 00_AXIOMS — Foundation Layer

Defines the fundamental worldview and assumptions of the system.

Includes:

- Universe as an information system  
- Consciousness as emergent complexity  
- Civilization as a cooperative intelligence network  

---

## 01_CONSENSUS — Constraint Layer (CCP)

Defines the non-negotiable constraints required for civilization survival.

Includes:

- prevention of systemic collapse  
- protection of intelligence ecosystems  
- preservation of shared reality  
- physical and infrastructural safeguards  

---

## 02_OPERATING_SYSTEM — Runtime Layer (COS)

Defines how civilization operates at a structural level.

### Components

- **civilization_operating_system.md**  
  → system architecture overview  

- **L3_coordination_layer.md**  
  → resource allocation and coordination  

- **L4_governance_layer.md**  
  → decision-making and control  

---

## 03_NCM — Value Layer

Defines how value is created and distributed.

### Components

- **NCM_protocol_v0.4.md**  
  → core value function  

- **NCM_execution_protocol_v0.1.md**  
  → execution constraints and anti-manipulation  

---

## Supporting Components

### docs/

Supporting materials and extended discussions.

---

## System Flow

```
Axioms
↓
Consensus (CCP)
↓
Operating System (COS)
↓
Value System (NCM)
↓
Execution &
